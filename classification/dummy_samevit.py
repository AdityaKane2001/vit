"""
Demonstration of having a ViT model with identical architecture as a pretrained 
model, but a different forward pass.

How timm works with models:

(For the sake of understanding, "original" models refer to the models defined by 
timm and having pretrained weights, and "new" models refer to user defined models
having identical architecture as the "original" models. "variant" refers to the
actual model of an architecture. Eg. "vit_tiny_patch16_224" is a variant of 
"VisionTransformer".)

    a. The original model file has the model definition class, and a `default_cgfs`
    dict. This dict has all pretrained configs for all model variants declared 
    in that file, if any, in the format "variant_name.pretraining_tag". 
    b. The `@register_model` decorator fetches these default configs in that 
    module, and registers all default configs for that variant. Other stuff,
    including the model entrypoint is also registered.
...
`boilerplate.py`, but not on `vit.py`.
"""
...
class MyAmazingAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        ##### Your amazing change here, for example
...
class MyAmazingBlock(nn.Module):
    def __init__(
            self,
            dim: int,
            num_heads: int,
...
class MyAmazingVisionTransformer(nn.Module):
    """ Vision Transformer
    ...
    def __init__(
            ...
            block_fn: Type[nn.Module] = MyAmazingBlock,
